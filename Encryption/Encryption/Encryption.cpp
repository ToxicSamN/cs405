...
std::string read_file(const std::string& filename)
{
  ...
  if (file.is_open()) {
      // read in the entire contents fo the file
      file.seekg(0, std::ios::end);
      file_text.reserve(file.tellg());
...
const std::string currentDateTime() {
    ...
    if (std::strftime(mbstr, sizeof(mbstr), "%Y-%m-%d", &timeinfo)) {
        return mbstr;
    }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    // get current timestamp from custom function currentDateTime()
    std::string currTimestamp = currentDateTime();
