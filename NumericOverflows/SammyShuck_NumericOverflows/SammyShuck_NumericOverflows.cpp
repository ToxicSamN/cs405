...
// OverflowException definition for handling overflow errors
struct OverflowException : public std::exception {
    const char* what() const throw() {
        return "overflow exception occurred";
    }
};

// UnderflowException definition for handling overflow errors
struct UnderflowException : public std::exception {
    const char* what() const throw() {
        return "underflow exception occurred";
    }
};
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {   
        // Check for overflow condition result += increment increaseing towards max
        if (increment > 0 && result > (std::numeric_limits<T>::max() - increment )) {
            throw OverflowException();
        }

        // Check for underflow condition result += -increment decreasing toward min
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // Check for overflow condition results -= -decrement is actually increasing towards max
        if ((decrement < 0) && (result > (std::numeric_limits<T>::max() + decrement))) {
            throw OverflowException();
        }

        // Check for underflow condition result -= decrement is decreasing towards min
...
void test_overflow()
{
    ...
    // Use try/catch to catch any overflow or underflow exceptions
    try {
        ...
        // output a message regarding overflow condition. Provide the maximum value allowed
        std::cout << e.what() << ", max allowed value: " << +(std::numeric_limits<T>::max()) << std::endl;
    }
    catch (UnderflowException& e) {
        // output a message regarding underflow condition. Provide the minimum value allowed
        std::cout << e.what() << ", min allowed value: " << +(std::numeric_limits<T>::min()) << std::endl;
    }
