...
#include <regex> // used for string pattern matching
...
// sqlInjection_detection is a function for parsing out sql injection
// patterns. It uses regex matching to identify the threat.
bool sqlInjection_detection(const std::string& sql) {
    try {
        ...
        std::transform(sqlCopy.begin(), sqlCopy.end(), sqlCopy.begin(), ::tolower);

        std::smatch match;
        std::regex_search(sqlCopy, match, regex_injection);

        // if any matches loop through and determine if a blind injection exists
        ...
        for (int i = 0; i < match.size(); ++i) {
            ...
            if (match[curr] == match[prev]) {
                // value=value
                // appears to be a blind sql injection
                // return true indicating found injection
        ...
        // something went wrong with regex
        // to be safe just return a false positive
        return true;
    }
}
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  char* error_message;
  if (sqlInjection_detection(sql)){
      const std::string injection_err = std::string("sql injection detected");

      std::cout << "Data failed to be queried from USERS table. ERROR = " << injection_err << std::endl;
